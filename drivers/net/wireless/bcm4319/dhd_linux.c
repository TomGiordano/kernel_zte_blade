...
struct timer_list bcm_suspend_timer;

typedef struct {
	void (*suspend_process)(void);
	void (*resume_process)(void);
}dhd_register_suspend;

extern void bcm_suspend_process(void);
extern void bcm_resume_process(void);

dhd_register_suspend  bcm_register_suspend = {
	.suspend_process = bcm_suspend_process,
	.resume_process = bcm_resume_process,
};

extern void mmc_register_dhd(void *func);
extern struct net_device *bcm_dev;
...
#define HW3 //add shaohua
#if defined(CONFIG_HAS_EARLYSUSPEND)

int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	int power_mode = PM_MAX;
	wl_pkt_filter_enable_t	enable_parm;  //add shaohua
	char iovbuf[32];
	int bcn_li_dtim = 3;
#ifdef HW3
	uint roamvar = 1;
...
extern wl_iw_t * bcm_iw;
extern void bcm_resume_in_dhd(void);
extern void bcm_suspend_in_dhd(void);

static void dhd_early_suspend(struct early_suspend *h)
{
	struct dhd_info *dhdp;

	/**** shaohua add, if softap funntion, do not change PM, packet fliter ****/
 	if(bcm_iw) {
		printk("shaohua softap, do not early_suspend \n");
		return;
	}
...
static void dhd_late_resume(struct early_suspend *h)
{
	...
	if(bcm_iw) {
		printk("shaohua softap, do not late_resume \n");
		return;
	}
	...
	printk("shaohua bcm_suspend_done %d \n", bcm_suspend_done);
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	dhdp->pktfilter[0] = "100 0 0 0 0x1 0x00";   //shaohua add :change 0xff to 0x1
#endif /* EMBEDDED_PLATFORM */
...
static void bcm_enable_mmchost_polling(int enable)
{
    mm_segment_t oldfs;
    struct file  *filp;
    int          length;
...
dhd_module_cleanup(void)
{
	...
	if(bcm_iw) {
		WAKE_UNLOCK(bcm_iw->pub, WAKE_LOCK_SOFTAP_SET);
        	WAKE_LOCK_DESTROY(bcm_iw->pub, WAKE_LOCK_SOFTAP_SET);
		bcm_iw = NULL;
		printk("shaohua clean lock \n");
	}	
	...
	bcm_enable_mmchost_polling(1);
	mdelay(100);
	bcm_enable_mmchost_polling(0);
}
...
dhd_module_init(void)
{
	...
	//mmc_register_dhd((void *)(&bcm_register_suspend));
	//printk("shaohua bcm_register %p, %p \n", bcm_register_suspend.suspend_process, bcm_register_suspend.resume_process);
