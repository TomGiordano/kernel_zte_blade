...
dhd_custom_get_mac_address(unsigned char *addr)
{
	...
	if(!rc) {
		...
		printk("shaohua get new mac success\n");
		
		return 0;
	} else {
 		printk("shaohua get mac fail \n");
...
#if 1
	bcm_mkiovar("btc_wire", (char *)&bt_wire, 4, iovbuf, sizeof(iovbuf));
        dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));

	bcm_mkiovar("btc_mode", (char *)&bt_mode, 4, iovbuf, sizeof(iovbuf));
        dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
