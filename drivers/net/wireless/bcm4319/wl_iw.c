...
wl_iw_t *bcm_iw = NULL;		//shaohua add for softap
#endif 
...
if(bcm_iw == NULL) {
	bcm_iw = iw;
	WAKE_LOCK_INIT(bcm_iw->pub, WAKE_LOCK_SOFTAP_SET, "SoftAP_SET");
	WAKE_LOCK(bcm_iw->pub, WAKE_LOCK_SOFTAP_SET);
	printk("shaohua set lock \n");
}
...
fail:
	...
	WL_SOFTAP(("%s exit with %d %p \n", __FUNCTION__, res, bcm_iw));
	return res;
}
#endif 
...
struct net_device *bcm_dev = NULL;
struct iw_request_info *bcm_info = NULL;
int bcm_suspend_done = 0;


void bcm_suspend_process(void)
{
	printk("shaohua enter bcm_suspend_process %p\n", bcm_dev);
	//msmsdcc_wifi_host_off();
	//wl_iw_control_wl_off(bcm_dev, bcm_info);
	...
	if(bcm_dev) {
		...
		printk("shaohua bcm_dev is NULL \n");
	}
}
EXPORT_SYMBOL(bcm_suspend_process);

void bcm_resume_process(void)
{
	printk("shaohua enter bcm_resume_process \n");
	
	//wl_iw_control_wl_on(bcm_dev, bcm_info);
...
EXPORT_SYMBOL(bcm_resume_process);

void bcm_resume_in_dhd(void)
{
...
EXPORT_SYMBOL(bcm_resume_in_dhd);

void bcm_suspend_in_dhd(void)
{

        printk("shaohua enter bcm_suspend_process \n");

	bcm_suspend_done = 1;
	wl_iw_control_wl_off(bcm_dev, bcm_info);
}

EXPORT_SYMBOL(bcm_suspend_in_dhd);
