 ...
 * @file        drivers/s5pc110/FSR_PAM_s5pc110.c
...
/*****************************************************************************/
/* ONDC specific definitions                                                 */
/*****************************************************************************/
#ifdef CONFIG_ONDC_DMA_ENABLE

PRIVATE UINT32	nONDC_REG_VirBaseAddr;

#include <asm/io.h>
#include <asm/cacheflush.h>

#define ONDC_REG_OFFSET			0x600000
#define ONDC_DMA_SRC_ADDR_OFFSET	0x400
#define ONDC_DMA_SRC_CFG_OFFSET		0x404
#define ONDC_DMA_DST_ADDR_OFFSET	0x408
#define ONDC_DMA_DST_CFG_OFFSET		0x40C
#define ONDC_DMA_TRANS_SIZE_OFFSET	0x414
#define ONDC_DMA_TRANS_CMD_OFFSET	0x418
#define ONDC_DMA_TRANS_STATUS_OFFSET	0x41C
#define ONDC_DMA_TRANS_DIR_OFFSET	0x420

/* Source Burst Length */
#define SRC_CFG_SBL_SINGLE		0x0	/* Single */
#define SRC_CFG_SBL_4BURST		0x2	/* 4 Burst */
#define SRC_CFG_SBL_8BURST		0x3	/* 8 Burst */
#define SRC_CFG_SBL_16BURST		0x4	/* 16 Burst */

/* Source Addressing Mode */
#define SRC_CFG_SAM_INC			0x0	/* Incremental addressing mode */
#define SRC_CFG_SAM_CNT			0x1	/* Constant addressing mode */

/* Source Data Width */
#define SRC_CFG_SDW_8BIT		0x0	/* 8 bit(byte) */
#define SRC_CFG_SDW_16BIT		0x1	/* 16 bit(half word) */
#define SRC_CFG_SDW_32BIT		0x2	/* 32 bit(word) */

/* Destination Burst Length */
#define DST_CFG_DBL_SINGLE		0x0	/* Single */
#define DST_CFG_DBL_4BURST		0x2	/* 4 Burst */
#define DST_CFG_DBL_8BURST		0x3	/* 8 Burst */
#define DST_CFG_DBL_16BURST		0x4	/* 16 Burst */

/* Destination Addressing Mode */
#define DST_CFG_DAM_INC			0x0	/* Incremental addressing mode */
#define DST_CFG_DAM_CNT			0x1	/* Constant addressing mode */

/* Destination Data Width */
#define DST_CFG_DDW_8BIT		0x0	/* 8 bit(byte) */
#define DST_CFG_DDW_16BIT		0x1	/* 16 bit(half word) */
#define DST_CFG_DDW_32BIT		0x2	/* 32 bit(word) */

#define set_src_cfg(sbl,sam,sdw)	((sbl<<16)|(sam<<8)|(sdw))
#define set_dst_cfg(dbl,dam,ddw)	((dbl<<16)|(dam<<8)|(ddw))

#define ONDC_DMA_SRC_CFG_READ	set_src_cfg(SRC_CFG_SBL_16BURST,SRC_CFG_SAM_INC,SRC_CFG_SDW_16BIT)
#define ONDC_DMA_DST_CFG_READ	set_src_cfg(DST_CFG_DBL_16BURST,DST_CFG_DAM_INC,DST_CFG_DDW_32BIT)
#define ONDC_DMA_SRC_CFG_WRITE	set_dst_cfg(SRC_CFG_SBL_16BURST,SRC_CFG_SAM_INC,SRC_CFG_SDW_32BIT)
#define ONDC_DMA_DST_CFG_WRITE	set_dst_cfg(DST_CFG_DBL_16BURST,DST_CFG_DAM_INC,DST_CFG_DDW_16BIT)

#define DMA_TRANS_CMD_TDC		(0x1<<18)
#define DMA_TRANS_CMD_TEC		(0x1<<16)
#define DMA_TRANS_CMD_TR		(0x1)

#define DMA_TRANS_STATUS_TD		(0x1<<18)
#define DMA_TRANS_STATUS_TB		(0x1<<17)
#define DMA_TRANS_STATUS_TE		(0x1<<16)

#define ONDC_DMA_DIR_READ		0x0
#define ONDC_DMA_DIR_WRITE		0x1

#define ONDC_DMA_SUCCESS		0x1
#define ONDC_DMA_FAIL			0x0
#endif
...
#ifdef CONFIG_ONDC_DMA_ENABLE
	nONDC_REG_VirBaseAddr = FSR_OAM_Pa2Va(FSR_ONENAND_PHY_BASE_ADDR + ONDC_REG_OFFSET);
        RTL_PRINT((TEXT("[PAM:   ]   OneNAND Controller DMA physical base address       : 0x%08x\r\n"), FSR_ONENAND_PHY_BASE_ADDR + 0x600000));
        RTL_PRINT((TEXT("[PAM:   ]   OneNAND Controller DMA virtual  base address       : 0x%08x\r\n"), nONDC_REG_VirBaseAddr));
#endif	
...
#ifdef CONFIG_ONDC_DMA_ENABLE
/*
 * FSR_PAM_ONDC_DMA_Start - [Internal] DMA transfer
 ...
 */
PRIVATE INT32
FSR_PAM_ONDC_DMA_Start(VOID *pSrc, VOID *pDest, UINT32 nSize, UINT32 nDirection)
{
	UINT32 DMA_CTL_BASE = nONDC_REG_VirBaseAddr;

	writel(pSrc, (DMA_CTL_BASE + ONDC_DMA_SRC_ADDR_OFFSET));		/* OND_4K_WRITE */
	writel(pDest, (DMA_CTL_BASE + ONDC_DMA_DST_ADDR_OFFSET));

	if (nDirection == ONDC_DMA_DIR_READ)
	{
		writel(ONDC_DMA_SRC_CFG_READ, (DMA_CTL_BASE + ONDC_DMA_SRC_CFG_OFFSET));	/* 16-Burst, Incremental, 16-bit */
		writel(ONDC_DMA_DST_CFG_READ, (DMA_CTL_BASE + ONDC_DMA_DST_CFG_OFFSET));	/* 16-Burst, Incremental, 32-bit */
	}
	else
	{
		writel(ONDC_DMA_SRC_CFG_WRITE, (DMA_CTL_BASE + ONDC_DMA_SRC_CFG_OFFSET));	/* 16-Burst, Incremental, 32-bit */
		writel(ONDC_DMA_DST_CFG_WRITE, (DMA_CTL_BASE + ONDC_DMA_DST_CFG_OFFSET));	/* 16-Burst, Incremental, 16-bit */
	}

	writel(nSize, (DMA_CTL_BASE + ONDC_DMA_TRANS_SIZE_OFFSET));
	writel(nDirection, (DMA_CTL_BASE + ONDC_DMA_TRANS_DIR_OFFSET));	/* Internal AHB to External AHB */

	writel(DMA_TRANS_CMD_TR, (DMA_CTL_BASE + ONDC_DMA_TRANS_CMD_OFFSET));

	return ONDC_DMA_SUCCESS;
}

/*
 * FSR_PAM_ONDC_DMA_Wait - [Internal] Check DMA status
 */
PRIVATE INT32
FSR_PAM_ONDC_DMA_Wait(void)
{
	UINT32 DMA_CTL_BASE = nONDC_REG_VirBaseAddr;
	UINT32 nStatus;
	...
	do
	{
		nStatus = readl(DMA_CTL_BASE + ONDC_DMA_TRANS_STATUS_OFFSET);
	} while (!(nStatus & DMA_TRANS_STATUS_TD));

	if (nStatus & DMA_TRANS_STATUS_TE)
	{
		writel(DMA_TRANS_CMD_TEC, (DMA_CTL_BASE + ONDC_DMA_TRANS_CMD_OFFSET));
		writel(DMA_TRANS_CMD_TDC, (DMA_CTL_BASE + ONDC_DMA_TRANS_CMD_OFFSET));
		RTL_PRINT((TEXT("[PAM:   ]   DMA Error!\r\n")));
		return ONDC_DMA_FAIL;
	}
...
#if 0
/*
 * FSR_PAM_LookupPTE - dump out the page tables associated with 'addr' in mm 'mm'
 * @param addr		virtual address
 */
unsigned long FSR_PAM_LookupPTE(unsigned long addr)
{
	pgd_t *pgd;
	pmd_t *pmd;
	pte_t *pte;
	...
	/*
	 * pgd[1:0] = 00 -> fault 
	 * pgd[1:0] = 01 -> page table
	 * pgd[1:0] = 10 -> section or supersection  
	 * pgd[1:0] = 11 -> Reserved
	 */

	if((pgd_val(*pgd) & 0x1) != 0x1)
	{
		return (addr & 0xfffff) | (pgd_val(*pgd) & 0xfff00000);
	}
                    ...
                    UINT32        nSize)
{
	...
	p_addr = FSR_PAM_LookupPTE((unsigned long)pSrc);

	FSR_PAM_ONDC_DMA_Start(
			(VOID *)p_addr,
		(VOID *)(FSR_ONENAND_PHY_BASE_ADDR | ((UINT32)pDst & 0x1FFFF)),
			nSize,
			ONDC_DMA_DIR_WRITE);
	FSR_PAM_ONDC_DMA_Wait();
#else
	/*
	 * Use memcpy src to kernel virtual address
	 * To Do: To avoid kmalloc and FSR_PAM_Memcpy
	 * by using VA-to-PA translation command of Cortex A8
	 * nSize is the number of bytes to transfer
	 */

	static UINT8* pTmpBuffer = NULL;

	if (pTmpBuffer == NULL)
		pTmpBuffer = kmalloc(nSize, GFP_KERNEL | GFP_DMA);

	FSR_PAM_Memcpy((VOID *)pTmpBuffer, (VOID *)pSrc, nSize);

	dmac_flush_range(pTmpBuffer, pTmpBuffer + nSize);

	FSR_PAM_ONDC_DMA_Start(
			(VOID *)virt_to_phys(pTmpBuffer),
			(VOID *)(FSR_ONENAND_PHY_BASE_ADDR | ((UINT32)pDst & 0x1FFFF)),
			nSize,
			ONDC_DMA_DIR_WRITE);
	FSR_PAM_ONDC_DMA_Wait();
