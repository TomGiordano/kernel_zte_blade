...
static void lcdc_lead_init(void)
{
	...
	/*
	gpio_lcd_lead_emuspi_write_one_index(0x2E80,0x00);//0x01 MTP ╪сть
	mdelay(120);
...
{
	unsigned int id_h,id_l,id,mask;
	...
	if(read_truly_id)
	{
		...
		SPI_Stop();
		SPI_Start();
		gpio_lcd_truly_emuspi_read_one_para(0xDB,&mask);
		SPI_Stop();
