			...
			unsigned long len, void *data)
{
	...
	if (table[page_no]) {
		//pr_info("less used_size: curr=%lu, dec=%u\n", used_size, ksize(table[page_no]));
	...
	//spin_lock(&write_lock);
        //relay_write(relay, buffer, len);

	// temp
	count++;
	//sprintf(buffer, "%lu %lu\n", count, used_size);		// count changes to weird value somewhere. why?
	sprintf(buffer, "%lu %lu %lu\n", count, used_size, used_size >> 10);	// correct result

        //sprintf(buffer, "%lu\n", used_size);
        relay_write(relay, buffer, strlen(buffer));

        //relay_write(relay, &used_size, sizeof(unsigned long));	// ouputs some wrong data! why?
...
{
	relay = relay_open("kmalloc_relay", NULL, RELAY_BUFFER_SIZE,
...
static int __init kt_init(void)
{
	...
	proc = create_proc_entry("kmalloc_test", 0666, NULL);
	if (proc == NULL) {
		vfree(table);
		return -ENOMEM;
	}
...
{
	unsigned long entry;
	remove_proc_entry("kmalloc_test", &proc_root);
        relay_close(relay);
