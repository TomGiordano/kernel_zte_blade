...
#define 	 AK8973B_RESET_GPIO		38
#else
#define      AK8973B_RESET_GPIO    	89
#endif
...
/*	ZTE_SENSOR_F.YUAO_001 2009-10-26	*/
	...
	if(1 == remove_compass){
		printk(KERN_INFO
		       	"akm8973_init_client: do not to request reset gpio \n");
	}
	else{
		#if 0
		...
		#endif
		printk(KERN_INFO
		       	"akm8973_init_client: do it in probe function\n");
	}
#if 0 
...
static int akm8973_init_client_remove_compass(struct i2c_client *client)
{
	mutex_init(&sense_data_mutex);
	init_waitqueue_head(&open_wq);

	/* As default, report accleration's data */
	atomic_set(&m_flag, 0);
	atomic_set(&a_flag, 1);
	atomic_set(&t_flag, 0);
	atomic_set(&mv_flag, 0);
...
static int akm8973_probe(struct i2c_client *client, const struct i2c_device_id * devid)
{
	...
	/*Here must reset ak8973B,and start it*/
	ret = gpio_request(AK8973B_RESET_GPIO, "akm8973");
	if (ret < 0) {
		printk(KERN_ERR
		       	"akm8973_init_client: request reset gpio failed\n");
		gpio_free(AK8973B_RESET_GPIO);
	}	

	ret = gpio_direction_output(AK8973B_RESET_GPIO, 0);
	mdelay(200);
	
	ret = gpio_direction_output(AK8973B_RESET_GPIO, 1);
	if (ret < 0) {
		printk(KERN_ERR
		       "akm8973_init_client: request reset gpio failed\n");
		gpio_free(AK8973B_RESET_GPIO);
	...
	if (err < 0){
		remove_compass = 1;
		printk(KERN_ERR
		       "akm8973_probe: Failed to set EEPROM access mode\n");
	}
	...
	if (err < 0){
		remove_compass = 1;
		printk(KERN_ERR
		       "akm8973_probe: Failed to read ETS from EEPROM\n");
	}
	...
	if (err < 0){
		remove_compass = 1;
		printk(KERN_ERR
		       "akm8973_probe: Do not set power down mode\n");
	}
	
	if(1 == remove_compass){
		akm8973_init_client_remove_compass(client);
		printk(KERN_INFO 
			"Compass is not exist. in %s\n",__func__);
	}
	else
	...
	if (err) {
		printk(KERN_ERR
			"Unable to register input device: %s in %s\n",akm->input_dev->name, __func__);
		goto exit_input_register_device_failed;
	} else
		printk(KERN_INFO 
			"Compass input devices register successful in %s\n",__func__);
...
static int __init akm8973_init(void)
{
	...
	if (ret) {
		printk(KERN_ERR 
			"akmd_device register failed in %s\n",__func__);
	} else
		printk(KERN_INFO 
			"akmd_device register successful in %s\n",__func__);

	ret = misc_register(&akm_aot_device);
	if (ret) {
		printk(KERN_ERR
			"akm_aot_device register failed in %s\n",__func__);
	} else
		printk(KERN_INFO 
			"akm_aot_device register successful in %s\n",__func__);
#if DEBUG
	printk(KERN_INFO "AK8973 compass driver: init\n");
#endif
