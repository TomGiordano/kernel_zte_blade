...
Geschichte:	                                                                        
Wenn               Wer          Was                                                                        Tag
2010-04-07    wangzy     Update ALS und prox source     
2010-04-09    wangzy     Interrupt-Bug zu umgehen                                     ZTE_ALSPRX_001
2010-04-12    wangzy     solve systemstart problem without  tsl2771            ZTE_ALSPRX_002
2010-04-13    wangzy     remove some comments                                          ZTE_ALSPRX_003
2010-04-16    wangzy     modify prox threshold                                                ZTE_ALSPRX_004
2010-04-21    wangzy     add prox threshold for 727A                                    ZTE_ALSPRX_005
2010-04-22    wangzy     add default prox threshld in case compile  error        ZTE_ALSPRX_006
2010-04-24    wangzy     solve als bug after dial                                             ZTE_ALSPRX_007
2010-05-15    wangzy     solve als_ioctl_on disturb audio problem                 ZTE_ALSPRX_008
2010-05-19    wangzy     add prox calibrate    function                                    ZTE_ALSPRX_009
2010-07-08    wangzy     modify prox threshold                                      ZTE_ALSPRX_010
******************************************************************/
// includes
...
static u16 prox_threshold_hi_param = 18000; 
static u16 prox_threshold_lo_param = 17500;
#else
...
static u8 prox_wait_time_param = 0xF6;//0xF2;
...
static void do_taos_work(struct work_struct *w)
{
	...
	{            
        ...
        //show threshold value for debug 
        /*
	...
	//printk(TAOS_TAG "prox_intr_threshold_hi_param=%d,prox_intr_threshold_lo_param=%d\n",prx_hi,prx_lo);
	*/
	if ((ret = (i2c_smbus_write_byte(taos_datap->client, (TAOS_TRITON_CMD_REG|TAOS_TRITON_CMD_SPL_FN|TAOS_TRITON_CMD_PROXALS_INTCLR)))) < 0) {        
	...
	switch (cmd) {
		...
		case TAOS_IOCTL_PROX_CALIBRATE:
			...
			for (i = 0; i < 20; i++) {
				...
				//pr_crit(TAOS_TAG "prox calibrate poll prox[%d] = %d\n",i,prox_cal_info[i].prox_data); 			
			...
			//if(prox_max>(12*prox_mean/10)){
				taos_cfgp->prox_threshold_hi = prox_mean+1500;//13*prox_mean/10;
				taos_cfgp->prox_threshold_lo = prox_mean+1000;//12*prox_mean/10;
			//}
			//else{
				//taos_cfgp->prox_threshold_hi = ((((prox_max - prox_mean) * 150) + 50)/100) + prox_mean;
				//taos_cfgp->prox_threshold_lo = ((((prox_max - prox_mean) * 130) + 50)/100) + prox_mean;
